...
const ContactForm: React.FC = () => {
  ...
  return (
    <div id="contact" className="flex flex-col lg:flex-row items-stretch justify-between p-8 lg:p-16 bg-background">
      {/* Left: Form Section */}
      <motion.div
        ref={formRef}
      ...
      >
        <h2 className="text-[#00adef] text-4xl font-bold mb-6 ">Contact Us</h2>
        <form className="space-y-4">
          ...
          <div>
            <label htmlFor="email" className=" text-grey-900 dark:text-white block text-sm font-medium text-foreground">
              Email
            </label>
            <input
              type="email"
          ...
          <div>
            <label htmlFor="message" className=" text-grey-900 dark:text-white block text-sm font-medium text-foreground">
              How can we help you?
            </label>
            <textarea
              id="message"
...
const Model: React.FC = () => {
  ...
  useFrame((state) => {
    if (modelRef.current) {
      modelRef.current.rotation.y = Math.sin(state.clock.elapsedTime * 0.5) * 0.5
    }
  })
